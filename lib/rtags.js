...
let format_references = function(out) {
  ...
  for (let line of out.split("\n")) {
    ...
    let [fn, row, col, ...strline] = line.split(":");
    strline = strline.join(':');
    [strline, parent_fn_sig, parent_fn_loc] = strline.split("\tfunction: ");
    let parent_details = null;
    if (parent_fn_loc) {
      let [parent_fn_file, parent_fn_line, parent_fn_col] = parent_fn_loc.split(":");
...
module.exports.RcExecutor = class RcExecutor extends EventEmitter {
  ...
  rc_exec(opt, input) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      try {
          this.rc_worker.once('message', handleMessage);
          this.rc_worker.send({cmd: rc_cmd, args: opt, input: input, id: this.messageId});
      }
      catch (e) {
        this.rc_worker = undefined;
        this.start_rc_worker();
        atom.notifications.addError("Rtags executor process restarted, please try again");
      }
  ...
  find_symbol_at_point(fn, loc) {
    let promise = this.rc_exec([`--current-file=${fn}`, '-f', fn_loc(fn, loc), '-K', '--all-targets']);
    return promise.then((out) => {
      let results = [];

      for (let line of out.split("\n")) {
        let col, row, newFn, context;
        [newFn, row, col, context] = line.split(":");
        results.push([newFn, row-1, col-1, context])
      }
      ...
      if (results.length > 1) {
        ...
        .then((editor) => {
          editor.setCursorBufferPosition(loc);
          editor.moveToBeginningOfWord();
          let initialPos = editor.getCursorBufferPosition();
          editor.moveToEndOfWord();
          let finalPos = editor.getCursorBufferPosition();
          let searchStr = editor.getTextInBufferRange(new Range(initialPos, finalPos));
          for (let result of results) {
            if (result[3] && result[3].includes(searchStr)) {
              return result;
            }
          }
          return results[0];
  ...
  index_unsaved_file(fn, current_content) {
      return this.rc_exec([`--unsaved-file=${fn}:${current_content.length}`, '--reindex', fn], current_content);
  }
  ...
  get_refactor_locations(fn, loc) {
    ...
    return promise.then(function(out) {
      ...
      for (let line of out.split('\n')) {
        ...
        [path, row, column] = line.split(':');
        if (ret[path] === undefined) {
          ret[path] = [];
        }
        ret[path].push(new Point(parseInt(row), parseInt(column)));
      }
      return ret;
      });
  }
  ...
  diagnose(filename) {
    return this.rc_exec(['--diagnose', filename]);
  }

  get_fixits(fn) {
    return this.rc_exec(['--fixits', fn]).then((out) => {
      let ret = [];
      for (let line of out.split('\n')) {
        let lineCol, length, fix;
        [lineCol, length, fix] = line.split(' ');
        let row, col;
        [row, col] = lineCol.split(':');
        ret.push({
          location: new Point(parseInt(row)-1, parseInt(col)-1),
