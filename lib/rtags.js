...
module.exports.RcExecutor = class RcExecutor extends EventEmitter {
  ...
  get_symbol_info(fn, loc) {
    let promise = this.rc_exec(['-z', '-K', '-U', fn_loc(fn, loc), '--no-context']);
    return promise.then(function(out) {
      let ret = {};
      let fileLoc;
      [fileLoc, ...out] = out.split('\n');
      for (let line of out) {
        let splitIdx = line.indexOf(': ');
        let k = line.slice(0, splitIdx-1 + 1).trim();
      ...
      {
        // Fix range
        let [begin, end] = ret["Range"].split("-");
        let retRange = new Range;

        {
          let [row, col] = begin.split(":");
          row = parseInt(row) - 1;
          col = parseInt(col) - 1;
          retRange.start = new Point(row, col);
        }

        {
          let [row, col] = end.split(":");
          row = parseInt(row) - 1;
          col = parseInt(col) - 1;
          retRange.end = new Point(row, col);
        }
...

      {
        ...
        let [filename, row, col] = fileLoc.split(':');
        ret.StartLoc = { filename, position: new Point(parseInt(row) - 1, parseInt(col) - 1) };
      }
